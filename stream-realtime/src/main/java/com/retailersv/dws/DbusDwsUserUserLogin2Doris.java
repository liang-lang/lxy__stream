...
import com.retailersv.domain.UserLoginBean;
...
public class DbusDwsUserUserLogin2Doris {
        ...
        SingleOutputStreamOperator<UserLoginBean> beanDS = keyedDS.process(
                new KeyedProcessFunction<String, JSONObject, UserLoginBean>() {
                    ...
                    public void processElement(JSONObject jsonObj, KeyedProcessFunction<String, JSONObject, UserLoginBean>.Context ctx, Collector<UserLoginBean> out) throws Exception {
                        ...
                        String curLoginDate = DateTimeUtils.tsToDate(ts);
                        ...
                        if (StringUtils.isNotEmpty(lastLoginDate)) {
                            ...
                            if (!lastLoginDate.equals(curLoginDate)) {
                                ...
                                Long day = (ts - DateTimeUtils.dateToTs(lastLoginDate)) / 1000 / 60 / 60 / 24;
                                if (day >= 8) {
                                    backCt = 1L;
                                }
                            }
...
//        beanDS.print();
        //TODO 6.开窗
        AllWindowedStream<UserLoginBean, TimeWindow> windowDS = beanDS.windowAll(TumblingProcessingTimeWindows.of(Time.seconds(1)));
        ...
        //TODO 8.将聚合结果写到Doris
        // 在beanDS后打印水印，验证水印是否随ts推进

        reduceDS.print();
        reduceDS
                .map(new BeanToJsonStrMapFunction<UserLoginBean>())
                .sinkTo(DorisUtils.getDorisSink("dws_user_user_login_window"));
